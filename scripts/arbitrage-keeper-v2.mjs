...
const executor = getAddress(required("NEXT_PUBLIC_ARBITRAGE_EXECUTOR_V2"));
...
async function mintThenSell(strategy, budget, bondState, protocolFeeBps, rewardBps) {
  ...
  const gross = reserveOut - reserveRequired;
  const net = ownerProfit(gross, protocolFeeBps, rewardBps);
...
async function tryExecute(strategyId, strategy, protocolFeeBps, rewardBps) {
  ...
  const available = [strategy.maxReserve, balance, allowance].reduce((left, right) => left < right ? left : right);
...
async function poll(protocolFeeBps, rewardBps) {
  try {
    ...
    for (let index = 0; index < results.length; index += 1) {
      ...
      const [owner, hToken, reserveToken, validUntil, active, maxReserve, minProfitReserve] = result.result;
      if (!active || BigInt(validUntil) < block.timestamp) continue;
      await tryExecute(ids[index], { owner, hToken, reserveToken, maxReserve, minProfitReserve }, protocolFeeBps, rewardBps);
    }
    return true;
  } catch (error) {
...
console.log(`[keeper] watching Reserve Token executor ${executor} on Base as ${account.address}`);
