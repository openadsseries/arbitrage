    ...
    <div className="automation-page">
      ...
      <section className="automation-list-section">
        ...
        {continuous.executions.length ? <div className="automation-history">
          ...
          const decimals = market?.reserveDecimals ?? 18;
          const symbol = market?.reserveSymbol ?? "Reserve";
          return <a href={`${CHAINS.base.explorerUrl}/tx/${execution.transactionHash}`} target="_blank" rel="noreferrer" key={`${execution.transactionHash}-${execution.strategyId}`}>
            <div className="automation-market"><Image src={tokenLogoUrl(strategy?.hToken ?? execution.reserveToken, CHAINS.base.id)} alt="" width={34} height={34} unoptimized /><span><strong>{market ? `${market.reserveSymbol} ↔ ${market.symbol}` : shortAddress(execution.reserveToken)}</strong><small>{execution.direction}</small></span></div>
            <strong className="positive">+{tokenAmount(execution.ownerProfitReserveRaw, decimals)} {symbol}</strong>
            <span>{tokenAmount(execution.amountInReserveRaw, decimals)} {symbol}</span>
