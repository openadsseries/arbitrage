...
function geckoUrl(network: string, token: string, embed = false) {
  const url = new URL(
    `https://www.geckoterminal.com/${network}/tokens/${token}`,
  );
...
export function MarketAssetDetail({
  ...
  const marketsConnected = Boolean(
    arbitrageReadiness?.originalMarket.ready &&
    arbitrageReadiness?.hypedMarket.ready,
  );
  ...
  return (
    <>
      <header className="market-pair-head">
        ...
        <div
          className="market-view-select"
          role="tablist"
          aria-label="Market view"
        >
          <button
            aria-selected={view === "arbitrage"}
            className={view === "arbitrage" ? "selected" : ""}
          ...
          <button
            aria-selected={view === "hyped"}
            className={view === "hyped" ? "selected" : ""}
            onClick={() => setView("hyped")}
            role="tab"
            type="button"
          >
            <Image
      ...
      {view === "arbitrage" && (
        <>
          <section
            className="arbitrage-focus market-workspace-face"
            aria-label={`${market.symbol} arbitrage`}
          >
              ...
              {marketsConnected ? (
                ...
                <div className="price-gap-view">
                  <div className="price-gap-heading">
                    <span className="kicker">Live quote</span>
                    <h2>Both markets are required.</h2>
                    <p>
                      GETHYPED needs an executable price for both tokens before
                      it can compare them.
                    </p>
                  </div>
                </div>
      ...
      {view !== "arbitrage" && (
        <section
          key={`trade-${asset}`}
          className="market-price-section with-trade market-workspace-face"
          aria-label={`Trade ${selected.symbol}`}
        >
          <div className="market-price-head">
            <div className="market-trade-title">
              <Image
                src={tokenLogoUrl(selected.address, chain.id)}
                alt=""
                width={38}
                height={38}
                unoptimized
          ...
          <div className="market-context-body">
            <section
              className="market-embed-section"
              aria-label={`${selected.symbol} live market chart`}
            >
              <div className="market-embed-bar">
                <span>{selected.symbol} live market</span>
                <a href={sourceUrl} target="_blank" rel="noreferrer">
                  Open on GeckoTerminal <ExternalLink />
                </a>
              </div>
            ...
            <aside
              className="market-buy-column"
              aria-label={`Trade ${selected.symbol}`}
            >
              {asset === "og" ? (
                <OgSwapPanel market={market} embedded />
              ) : (
                <QuickBuyPanel market={market} embedded />
              )}
            </aside>
          </div>
        </section>
