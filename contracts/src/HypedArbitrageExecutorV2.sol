...
/// @notice Executes one of two fixed Reserve -> Hyped Token -> Reserve arbitrage paths.
/// @dev WETH is only an internal routing asset. User capital, profit and executor reward are
///      always accounted for and paid in the Mint Club reserve token.
contract HypedArbitrageExecutorV2 {
    ...
    // One active permission per wallet and reserve token keeps ERC-20 approval accounting clear.
    ...
    /// @dev A V4 pool can settle its WETH-facing side as native ETH. Only ETH received by the
    ///      current swap is wrapped; pre-existing ETH is never included in an execution.
    ...
    function startStrategy(
        ...
        if (
            hToken == address(0) || maxReservePerExecution == 0 || minProfitReserve == 0
    ...
    function stopStrategy(uint256 strategyId) external {
        ...
        if (activeStrategyId[msg.sender][strategy.reserveToken] == strategyId) {
            delete activeStrategyId[msg.sender][strategy.reserveToken];
        }
        ...
        returns (uint256 ownerReturnReserve)
    {
        ...
        if (params.amountInReserve == 0 || params.amountInReserve > strategy.maxReservePerExecution)
        {
    ...
    function _mintThenSell(
        ...
        // A non-standard reserve transfer during minting must not silently alter accounting.
