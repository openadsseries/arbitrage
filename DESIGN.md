...
- Make task order explicit with `1. Check profit`, `2 · Start arbitrage` and `3. Position history`. The current profit state is the primary result, followed by the historical price comparison, the compact arbitrage path and the exact returned Reserve Token amount.
...
- Give the primary authorization form one editable Reserve Token amount and one `Start arbitrage` action; after signing, use `Authorized`. Direction, route, per-execution sizing and cumulative limits remain execution details, not user inputs.
