...
- Give the primary authorization form one editable Reserve Token amount and one `Execute arbitrage` action. After signing, try execution immediately; if price moves first, show `Watching` with `Execute now` and `Stop`. Direction, route, per-execution sizing and cumulative limits remain execution details, not user inputs.
