...
- Show only facts derived from current route readiness. Never claim that an opportunity or profit exists until the same-block closed-loop quote remains net-positive after price impact, exchange fees and the successful-executor reward. The executor verifies its gas coverage again at execution.
...
- A price-gap result must come from current executable Uniswap and Mint Club quotes at the user's bounded budget. Show amount in, amount returned and the user's net after exchange fees and the successful-executor reward. State that gas is verified at execution; never claim gas is already deducted from the quote.
...
- Make task order explicit with `1 · Check profit` over the system-calculated result and `2 · Start arbitrage` over the user's action. The current profit state is the primary result, followed by the historical price comparison, best quoted route and exact return.
...
- Align `1 · Live quote` at the left and `Current opportunity` with the live percentage at the right of the same result header. Label the route `Best quoted route`, stretch it across the comparison width, and place its input and return amounts at the two endpoints.
- Do not repeat `Use`, `Return` and profit in a second result-card row. Put the input and returned WETH amounts at the two ends of the executable route, and place the estimated user profit directly under the opportunity percentage.
- Give the primary authorization form one editable budget and one `Start arbitrage` action; after signing, use `Authorized`. Direction and route remain execution details, not user inputs.
- Read the connected wallet's actual WETH and ETH balances. Use WETH first and convert only the missing ETH amount to WETH before authorization, while reserving native ETH for Base gas. State this beneath the budget without introducing a separate asset selector.
...
- Keep the primary Arbitrage form free of settings beyond its Reserve Token budget. One authorization permits one successful execution within 24 hours, requires a budget-scaled positive protected profit, and grants only the entered Reserve Token permission. WETH is an internal routing asset, never the user's accounting unit. Keep the two route directions and the 20% successful-executor reward in `Details`.
...
- After activation, replace the form in place with `Authorized`, the limit, expiry and one `Stop` action. Never use language such as `running`, `watching` or `automatic` unless a verified executor is actually monitoring and able to submit the transaction. A successful execution ends the authorization automatically because its exact allowance is consumed. Stopping before execution is one wallet action; removing the remaining token permission is a separate optional safety action.
